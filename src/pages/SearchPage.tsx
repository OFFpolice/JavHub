    ...
    <div className="w-full space-y-6">
      ...
      {isLoading ? (
        <div className="py-24 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-xs text-slate-400">Поиск в базе данных server.apijav.com...</p>
        </div>
      ) : posts.length === 0 ? (
        <div className="py-20 text-center p-8 bg-slate-900 border border-slate-800 rounded-2xl">
          ...
          <p className="text-xs text-slate-500 mb-4 max-w-sm mx-auto">
            {searchQuery
              ? `По запросу «${searchQuery}» совпадений не обнаружено. Попробуйте изменить формулировку.`
              : 'Введите ключевое слово или код видео в поле выше, чтобы найти нужные ролики.'}
          </p>
          {searchQuery && (
            <button
              onClick={handleClear}
