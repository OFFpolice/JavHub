...
export const ActressesPage: React.FC<ActressesPageProps> = ({
  selectedActor,
  onSelectActor,
  onClearActor,
  allActresses,
  ...
  // Filter actresses alphabetically & by search
  const filteredActresses = useMemo(() => {
    return allActresses.filter((item) => {
  ...
  }, [allActresses, search, selectedLetter]);

  // Mode 1: A specific actress is selected -> show her videos
  if (selectedActor) {
    return (
      <div className="w-full space-y-6">
        ...
        {isLoading ? (
          <div className="py-24 flex flex-col items-center justify-center gap-3">
            ...
            <p className="text-xs text-slate-400">Загрузка видео с {selectedActor}...</p>
          </div>
        ) : posts.length === 0 ? (
          <div className="py-20 text-center p-8 bg-slate-900 border border-slate-800 rounded-2xl">
            ...
            <p className="text-xs text-slate-500 mb-4 max-w-sm mx-auto">
              Пока нет опубликованных видео для этой актрисы в текущем запросе.
            </p>
            <button
    ...
    <div className="w-full space-y-6">
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl sm:text-2xl font-bold text-white tracking-tight">
              Каталог актрис и моделей
            </h1>
          </div>
          <p className="text-xs sm:text-sm text-slate-400 max-w-2xl">
            Полный реестр исполнительниц. Нажмите на имя для перехода ко всем видео с её участием.
          </p>
        </div>

        <div className="text-xs text-slate-400 tabular-nums self-end sm:self-center">
          Всего в базе: <strong className="text-slate-200">{allActresses.length.toLocaleString()}</strong>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <div className="relative max-w-md">
          ...
          <input
            ...
            placeholder="Поиск по имени актрисы..."
            className="w-full pl-10 pr-10 py-2.5 bg-slate-900 border border-slate-800 focus:border-purple-500 rounded-xl text-sm text-slate-100 placeholder:text-slate-500 focus:outline-none"
          />
          {search && (
            <button
              onClick={() => setSearch('')}
      ...
      {taxonomiesLoading ? (
        <div className="py-24 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-xs text-slate-400">Загрузка каталога актрис из API...</p>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2.5">
          {filteredActresses.slice(0, 300).map((actress) => (
            <button
              key={actress}
              onClick={() => onSelectActor(actress)}
              className="flex items-center justify-between p-3 bg-slate-900 hover:bg-slate-800/90 border border-slate-800 hover:border-purple-500/50 rounded-xl text-left transition-all group cursor-pointer"
            >
              <div className="flex items-center gap-2.5 min-w-0">
                <span className="w-2 h-2 rounded-full bg-purple-500/50 group-hover:bg-purple-400 group-hover:scale-125 transition-all shrink-0" />
                <span className="text-xs sm:text-sm font-medium text-slate-200 group-hover:text-white truncate">
                  {actress}
                </span>
              </div>
              <ArrowRight className="w-3.5 h-3.5 text-slate-600 group-hover:text-purple-400 group-hover:translate-x-0.5 transition-all shrink-0 ml-1.5" />
            </button>
          ))}
        </div>
      )}
